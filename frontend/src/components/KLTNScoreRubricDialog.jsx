...
import {
  KLTN_RUBRIC_CRITERIA,
  KLTN_RUBRIC_MAX_TOTAL,
  clampRubricValues,
  emptyRubricValues,
  sumRubric
} from "../config/kltnRubric";

export default function KLTNScoreRubricDialog({
  open,
  onClose,
  onSave,
  studentLabel,
  ...
  const [vals, setVals] = useState(() => emptyRubricValues());
  const [note, setNote] = useState("");

  useEffect(() => {
    if (!open) return;
    if (initialValues && initialValues.length === KLTN_RUBRIC_CRITERIA.length) {
      setVals(initialValues.map((v, i) => (v === "" || v === null || v === undefined ? "" : String(v))));
    } else {
      setVals(emptyRubricValues());
    }
    setNote(initialNote || "");
  }, [open, initialValues, initialNote]);

  const numeric = clampRubricValues(vals.map((v) => (v === "" ? 0 : v)));
  const total = sumRubric(vals);
  ...
  function handleBlur(i) {
    setVals((prev) => {
      const next = [...prev];
      const max = KLTN_RUBRIC_CRITERIA[i].max;
      let v = Number(next[i]);
          ...
          <Table size="small">
            ...
            <TableBody>
              ...
              <TableRow sx={{ bgcolor: alpha(accent, 0.06) }}>
                ...
                <TableCell align="center">
                  <Typography sx={{ fontWeight: 800, color: accent, fontSize: "1.1rem" }}>
                    {total.toFixed(2)}
                  </Typography>
                </TableCell>
              </TableRow>
